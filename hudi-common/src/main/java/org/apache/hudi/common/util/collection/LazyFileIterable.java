...
public class LazyFileIterable<T, R> implements Iterable<R> {
  ...
  public LazyFileIterable(String filePath, Map<T, DiskBasedMap.ValueMetadata> map, boolean isCompressionEnabled) {
  ...
  public class LazyFileIterator<T, R> implements Iterator<R> {
    ...
    public R next() {
      ...
      return DiskBasedMap.get(entry.getValue(), readOnlyFileHandle, isCompressionEnabled);
    }
